...
def _migrate() -> None:
    ...
    for migration in sorted((ROOT / "migrations").glob("*.sql")):
        ...
        found = _rows(
            "SELECT Checksum FROM Agentic.AI_SCHEMA_MIGRATION WHERE Version = ?", (migration.name,)
        )
        if found:
...
def _import_contract() -> int:
    ...
    for operation in contract.operations:
        ...
        if not _rows(
            ...
            (operation.stable_key, contract.source_hash),
        ):
            _sql(
                ...
                (
                    ...
                    json.dumps(operation.request_body),
                    json.dumps(operation.responses),
                    json.dumps(operation.security),
                    "UNKNOWN",
                    "UNREVIEWED",
                    0,
                    datetime.now(timezone.utc).isoformat(),
...
def _seed_reviewed_read_defaults() -> int:
    ...
    checksum = hashlib.sha256(
        "\n".join(sorted(item["stable_key"] for item in defaults)).encode("utf-8")
    ).hexdigest()
    ...
    try:
        for item in defaults:
            ...
            if not rows:
                raise RuntimeError(
                    f"Reviewed read tool was not imported: {item['method']} {item['path']}"
                )

...
class AgentRepository:
    def list_agents(self):
        rows = _rows(
            "SELECT TOP 200 ID,ConfigJSON,Revision,ActiveRun,UpdatedAt FROM Agentic.MVP_AGENT ORDER BY Name,ID"
        )
        return [
            dict(json.loads(r[1]), id=r[0], revision=r[2], active_run=r[3], updated_at=r[4])
            for r in rows
        ]

    def get_agent(self, identifier):
        rows = _rows(
            "SELECT ConfigJSON,Revision,ActiveRun,UpdatedAt FROM Agentic.MVP_AGENT WHERE ID=?",
            (identifier,),
        )
        if not rows:
    ...
    def save_agent(self, config, actor, identifier=None, revision=None):
        with transaction():
            if identifier:
                ...
                _sql(
                    "UPDATE Agentic.MVP_AGENT SET Name=?,ConfigJSON=?,Revision=Revision+1,Enabled=?,IntervalSeconds=?,NextDue=?,UpdatedBy=?,UpdatedAt=? WHERE ID=?",
                    (
                        config["name"],
                        json.dumps(config),
                        int(config["enabled"]),
                        config["interval_seconds"],
                        time.time() + config["interval_seconds"],
                        actor,
                        now(),
                        identifier,
                    ),
                ...
                _sql(
                    "INSERT INTO Agentic.MVP_AGENT (ID,Name,ConfigJSON,Revision,Enabled,IntervalSeconds,NextDue,UpdatedBy,UpdatedAt) VALUES (?,?,?,?,?,?,?,?,?)",
                    (
                        identifier,
                        config["name"],
                        json.dumps(config),
    ...
    def enqueue(self, identifier, actor, trigger="MANUAL"):
        with transaction():
            ...
            if trigger == "SCHEDULE":
                due = _rows(
                    "SELECT NextDue,IntervalSeconds FROM Agentic.MVP_AGENT WHERE ID=?",
                    (identifier,),
                )[0]
            ...
            run_id = str(uuid4())
            _sql(
                "INSERT INTO Agentic.MVP_RUN (ID,AgentID,SnapshotJSON,State,TriggerKind,Actor,CreatedAt) VALUES (?,?,?,?,?,?,?)",
                (run_id, identifier, json.dumps(agent), "QUEUED", trigger, actor, now()),
            )
            _sql(
                "UPDATE Agentic.MVP_AGENT SET ActiveRun=?,NextDue=? WHERE ID=?",
    ...
    def schedule(self):
        rows = _rows(
            "SELECT TOP 20 ID FROM Agentic.MVP_AGENT WHERE Enabled=1 AND IntervalSeconds>0 AND NextDue<=? AND ActiveRun IS NULL ORDER BY NextDue",
            (time.time(),),
        )
        for row in rows:
    ...
    def claim(self):
        with transaction():
            rows = _rows(
                "SELECT TOP 1 ID,AgentID FROM Agentic.MVP_RUN WHERE State='QUEUED' ORDER BY CreatedAt"
            )
            if not rows:
                return None
            ...
            if not agent["enabled"]:
                _sql(
                    "UPDATE Agentic.MVP_RUN SET State='CANCELLED',FinishedAt=? WHERE ID=?",
                    (now(), identifier),
                )
                _sql(
                ...
                return None
            _sql(
                "UPDATE Agentic.MVP_RUN SET State='RUNNING',StartedEpoch=? WHERE ID=?",
    ...
    def get_run(self, identifier):
        rows = _rows(
            "SELECT AgentID,SnapshotJSON,State,TriggerKind,Actor,CreatedAt,FinishedAt,Report,ErrorCode FROM Agentic.MVP_RUN WHERE ID=?",
            (identifier,),
        )
        if not rows:
        ...
        result["calls"] = [
            dict(
                id=c[0],
                tool=c[1],
                arguments=json.loads(c[2]),
                result=json.loads(c[3]),
                outcome=c[4],
                created_at=c[5],
            )
            for c in _rows(
                "SELECT ID,ToolKey,ArgumentsJSON,ResultJSON,Outcome,CreatedAt FROM Agentic.MVP_CALL WHERE RunID=? ORDER BY CreatedAt,ID",
                (identifier,),
            )
        ]
        return result
    ...
    def list_runs(self):
        return [
            dict(
                zip(
                    ("id", "agent_id", "state", "created_at", "finished_at", "error", "agent_name"),
                    r,
                    strict=True,
                )
            )
            for r in _rows(
                "SELECT TOP 100 r.ID,r.AgentID,r.State,r.CreatedAt,r.FinishedAt,r.ErrorCode,a.Name FROM Agentic.MVP_RUN r JOIN Agentic.MVP_AGENT a ON a.ID=r.AgentID ORDER BY r.CreatedAt DESC"
            )
        ]
    ...
    def record_call(self, run_id, key, arguments, result, outcome):
        identifier = str(uuid4())
        _sql(
            "INSERT INTO Agentic.MVP_CALL (ID,RunID,ToolKey,ArgumentsJSON,ResultJSON,Outcome,CreatedAt) VALUES (?,?,?,?,?,?,?)",

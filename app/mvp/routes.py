...
def tool_catalog():
    ...
    if not items:
        return items
    availability = current_app.extensions["agentic_repository"].list_tool_availability(
        items[0]["contract_hash"]
    )
    for item in items:
...
def set_tool_availability(stable_key):
    ...
    if not item["supported"]:
        return jsonify(
            error={"message": "This operation uses an unsupported OpenAPI request shape."}
        ), 409
    if (
        enabled
        and item["method"] in {"POST", "PUT", "PATCH", "DELETE"}
        and payload.get("acknowledge_auto_execution") is not True
    ):
        return jsonify(
            error={"message": "Explicitly acknowledge automatic execution for mutating operations."}
        ), 400
    if enabled and item["sensitive"] and payload.get("acknowledge_sensitive_data") is not True:
        return jsonify(
            error={"message": "Explicitly acknowledge sensitive data handling for this operation."}
        ), 400
...
def update(identifier):
    ...
    return jsonify(
        repository().save_agent(
            config, current_principal().name, identifier, payload.get("revision")
        )
    )

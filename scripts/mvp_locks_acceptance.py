...
required_tools = [
    next(tool for tool in catalog if tool["key"] == key)
    for key in ("get_v2_locks", "get_v2_process")
]
...
try:
    for tool in required_tools:
        if not tool["available"]:
            request(
                "PUT",
                f"/api/mvp/catalog/{tool['stable_key']}/availability",
                {
                    "enabled": True,
    ...
    agent = request(
        ...
        "/api/mvp/agents",
        dict(
            name="__MVP_LOCKS_ACCEPTANCE__",
            prompt="You are a DBA. Use tools to produce a concise, factual report in English.",
            ...
            interval_seconds=0,
            tools=[
                {"key": "get_v2_locks", "fixed": {"filter": "AgenticMVPTest", "maxRows": 100}},
                {"key": "get_v2_process", "fixed": {}},
            ],
        ),
        status=201,
    )
    ...
    locks = [
        c for c in result["calls"] if c["tool"] == "get_v2_locks" and c["outcome"] == "SUCCEEDED"
    ]
    processes = [
        c for c in result["calls"] if c["tool"] == "get_v2_process" and c["outcome"] == "SUCCEEDED"
    ]

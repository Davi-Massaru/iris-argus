...
def call(method, path, payload=None, status=200) -> dict[str, Any]:
...
locks_tool = next(
    tool for tool in call("GET", "/api/mvp/catalog")["items"] if tool["key"] == "get_v2_locks"
)
...
if not locks_tool["available"]:
    call(
        "PUT",
        f"/api/mvp/catalog/{locks_tool['stable_key']}/availability",
        {
            "enabled": True,
...
config: dict[str, Any] = dict(
    name="__MVP_SMOKE__",
    prompt="You are a DBA. Use the tools and report only observed evidence.",
    task="Query get_v2_locks and report how many rows were returned. Keep the report brief and in English.",
    model=os.environ.get("AGENTIC_MODEL", "qwen2.5:3b"),
    provider=os.environ.get("AGENTIC_PROVIDER", "ollama"),
    enabled=True,
    interval_seconds=0,
...
agent = call(
    "PUT",
    "/api/mvp/agents/" + agent["id"],
    {**agent, "prompt": config["prompt"] + " Cite evidence."},
)
...
print(
    ...
    result["state"],
    result["error"],
    [(c["tool"], c["outcome"]) for c in result["calls"]],
    flush=True,
)
assert result["state"] == "SUCCEEDED", result["error"]
assert result["report"] and any(
    c["tool"] == "get_v2_locks" and c["outcome"] == "SUCCEEDED" for c in result["calls"]
)
...
fixture_path.write_text(json.dumps(fixture))
print(
    "MVP_HTTP_OK create, edit, revision conflict, queue dedup, provider, SysAdmin, report, pause",
    flush=True,
)

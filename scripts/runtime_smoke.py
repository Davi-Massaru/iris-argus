...
def main():
    ...
    csrf = requests.post(
        base + "/api/v1/proposals/nonexistent/approve",
        auth=admin,
        json={"revision": 1, "action_hash": "a" * 64},
        timeout=10,
    )
    assert csrf.status_code == 403
    print(
        "HTTP_SMOKE_OK identity, anonymous/unauthorized denial, SQL pages, assets, CSRF, worker readiness"
    )

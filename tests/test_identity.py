...
def test_admin_name_does_not_grant_privilege():
    ...
    assert (
        client.get("/api/v1/overview", environ_base={"REMOTE_USER": "_SYSTEM"}).status_code == 403
    )
...
def test_runtime_uses_native_security_context_not_http_headers(monkeypatch):
    ...
    response = app.test_client().get(
        "/api/v1/session",
        environ_base={"REMOTE_USER": "_SYSTEM"},
        headers={"X-Test-User": "_SYSTEM"},
    )
...
def test_security_context_failure_denies_access(monkeypatch):
    ...
    assert (
        client.get("/api/v1/overview", environ_base={"REMOTE_USER": "_SYSTEM"}).status_code == 401
    )
...
def test_unknown_user_denied_even_with_roles(monkeypatch):
    monkeypatch.setitem(
        sys.modules,
        "iris",
        SimpleNamespace(execute=lambda q: "UnknownUser" if "username" in q else "%All"),
    )

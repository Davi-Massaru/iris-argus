...
def config():
    ...
    return dict(name='DBA test', prompt='Inspect the environment.', task='Query locks and report the findings.',
                model='qwen3:4b', provider='ollama', enabled=True, interval_seconds=0,
                tools=[{'key': tool['key'], 'stable_key': tool['stable_key'],
                        'contract_hash': tool['contract_hash'], 'fixed': {'maxRows': 100}}])
...
def test_sensitive_values_are_redacted_and_schema_examples_removed():
    assert redact_sensitive({'user': 'dba', 'nested': [{'password': 'secret', 'apiToken': 'token'}]}) == {
        'user': 'dba', 'nested': [{'password': '[REDACTED]', 'apiToken': '[REDACTED]'}]
    }
    assert _remove_schema_examples({'type': 'string', 'example': 'SYS', 'default': 'secret'}) == {'type': 'string'}
...
def test_gateway_dispatches_json_body_and_accepts_created_response(monkeypatch):
    ...
    class Response:
        ...
        def __enter__(self): return self
        def __exit__(self, *args): pass
        def iter_content(self, size): yield b'{"created": true}'
    ...
    gateway = Gateway([{
        ...
        'fixed': {},
    }], Transport(), availability_lookup=lambda _item: True)
...
def test_runtime_records_evidence_and_uses_saved_prompt():
    repo = FakeRepo()
    model = Model([AIMessage(content='', tool_calls=[{'id':'c1','name':'get_v2_locks','args':{}}]), AIMessage(content='No locks. Evidence evidence-1.')])
    class ReadGateway:
...
def test_runtime_rebinds_legacy_agent_to_deployment_provider(monkeypatch):
    ...
    class NoEvidenceModel:
        ...
        def invoke(self, messages): return AIMessage(content='No evidence yet.')

    with pytest.raises(ToolError, match='NO_SUCCESSFUL_TOOL_EVIDENCE'):
        execute('run-1', repo, lambda provider, model: (selected.append((provider, model)) or NoEvidenceModel()), lambda _bindings: None)
...
def test_runtime_never_accepts_report_without_tool_evidence():
    repo = FakeRepo()
    with pytest.raises(ToolError, match='NO_SUCCESSFUL_TOOL_EVIDENCE'):
        execute('run-1', repo, lambda *_:Model([AIMessage(content='Invented report')]), lambda *_:None)
...
def test_tool_availability_toggle_requires_dba_csrf_and_acknowledgment():
    ...
    }, headers=headers, environ_base=dba)
